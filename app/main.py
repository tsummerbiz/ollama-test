...
async def download_file(
    ...
    # 2. タスクの結果からファイルパスを取得
    # aggregate_results の戻り値 {"final_file": "..."} を参照
    result_data = task_result.result
    file_path_str = result_data.get("final_file")
    if not file_path_str or not os.path.exists(file_path_str):
        raise HTTPException(status_code=404, detail="Result file not found.")

    try:
        # 3. ヘルパー関数を使用して復号
        # サーバーのディスクに復号済みファイルを書き出さず、メモリ(io.StringIO)上で処理
...
async def get_status(task_id: str):
    ...
    if task_result.ready() and task_result.status == "SUCCESS":
        # ダウンロード用のURLをヒントとして含める
        response["download_url"] = f"/download/{task_id}?password=YOUR_PASSWORD"
